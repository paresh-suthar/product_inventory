...
def generate_invoice_pdf(invoice, client, bank_account, subscription=None, server=None) -> bytes:
    ...
    title_style = ParagraphStyle(
        "InvoiceTitle", parent=styles["Heading1"], fontSize=20, leading=24, textColor=colors.HexColor("#1e3a8a")
    )
    header_style = ParagraphStyle(
        "HeaderStyle", parent=styles["Normal"], fontSize=9, leading=12, textColor=colors.HexColor("#475569")
    )
    ...
    header_data = [
        [
            Paragraph(
                "<b>StockFlow Cloud ERP</b><br/>Cloud & Server Infrastructure Management<br/>Email: billing@stockflow.internal",
                header_style,
            ),
            Paragraph(
                f"<b>TAX INVOICE</b><br/>Invoice No: <b>{invoice.invoice_no}</b><br/>Status: <b>{invoice.status}</b>",
                title_style,
            ),
        ]
    ]
    ...
    bill_info = [
        [
            Paragraph(
                f"<b>BILLED TO:</b><br/><b>{client.company_name}</b><br/>Attn: {client.contact_name}<br/>Email: {client.email}<br/>Address: {client.billing_address or 'N/A'}<br/>Tax ID: {client.tax_id or 'N/A'}",
                header_style,
            ),
            Paragraph(
                f"<b>INVOICE DETAILS:</b><br/>Issue Date: <b>{issue_str}</b><br/>Due Date: <b>{due_str}</b><br/>Currency: <b>{invoice.currency}</b>",
                header_style,
            ),
        ]
    ]
    ...
    server_desc = (
        f"Server: {server.hostname} ({server.datacenter_location}) | IP: {server.primary_ip}"
        if server
        else "Cloud Infrastructure Hosting Service"
    )

    items_data = [
        [
            Paragraph("<b>Description</b>", bold_style),
            Paragraph("<b>Billing Cycle</b>", bold_style),
            Paragraph("<b>Amount</b>", bold_style),
        ],
        [
            Paragraph(f"<b>{plan_desc}</b><br/>{server_desc}", header_style),
            Paragraph(subscription.billing_cycle if subscription else "MONTHLY", header_style),
            Paragraph(f"{invoice.currency} {float(invoice.subtotal):,.2f}", bold_style),
        ],
        [
            "",
            Paragraph("<b>Tax / VAT:</b>", header_style),
            Paragraph(f"{invoice.currency} {float(invoice.tax_amount):,.2f}", header_style),
        ],
        [
            "",
            Paragraph("<b>TOTAL DUE:</b>", bold_style),
            Paragraph(f"<b>{invoice.currency} {float(invoice.total_amount):,.2f}</b>", title_style),
        ],
    ]
