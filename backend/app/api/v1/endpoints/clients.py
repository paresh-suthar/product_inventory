...
from app.schemas.client import ClientCreate, ClientUpdate, ClientResponse, WalletDepositRequest, ClientWalletResponse
...
async def list_clients(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Client).options(
            selectinload(Client.wallet).selectinload(ClientWallet.transactions)
...
async def create_client(req: ClientCreate, db: AsyncSession = Depends(get_db)):
    ...
    wallet = ClientWallet(
        ...
        balance=req.initial_wallet_deposit or 0.0
    )
    db.add(wallet)
